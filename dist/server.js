...
class HarReplayServer {
    constructor(initialSources, storageDir = './har_storage', port = 3000) {
        this.harDataMap = new Map();
        this.loadedHarFiles = [];
        this.lastReloadTimestamp = null;
        this.METHODS_WITH_BODY = new Set(['POST', 'PUT', 'PATCH', 'DELETE']);
        this.requestHandler = (req, res) => __awaiter(this, void 0, void 0, function* () {
            var _a;
            const url = new URL(req.url || '/', `http://${req.headers.host}`);
            const method = ((_a = req.method) === null || _a === void 0 ? void 0 : _a.toUpperCase()) || 'GET';
        ...
        this._replayRequest = (req, res) => __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            try {
                const method = ((_a = req.method) === null || _a === void 0 ? void 0 : _a.toUpperCase()) || 'GET';
                ...
                const replayState = (_b = this.harDataMap.get(method)) === null || _b === void 0 ? void 0 : _b.get(urlPath);
                if (!replayState || replayState.entries.length === 0) {
                    console.warn(`[NO MATCH] No candidate found for path: ${method} ${urlPath}`);
                    if (!res.headersSent)
                        res.writeHead(404);
                    if (!res.writableEnded)
                        res.end(`No entry found for path: ${method} ${urlPath}`);
                    return;
                }
                let priorityMatch = null;
                if (this.METHODS_WITH_BODY.has(method)) {
                    const reqBody = yield this.getRequestBody(req);
    ...
    colorizeUrlPath(method, urlPath) {
        ...
        if (this.METHODS_WITH_BODY.has(method) || (method === 'GET' && ext === ''))
            return chalk.green(urlPath);
        if (IMAGE_EXTS.has(ext))
            return chalk.magenta(urlPath);
        if (DOC_EXTS.has(ext))
            return chalk.blue(urlPath);
        if (SCRIPT_EXTS.has(ext))
            return chalk.cyan(urlPath);
        if (FONT_EXTS.has(ext))
            return chalk.yellow(urlPath);
        if (method === 'GET')
            return chalk.green(urlPath);
        return urlPath;
    }
    ...
    compareURLSearchParams(p1, p2) {
        p1.sort();
        p2.sort();
        return p1.toString() === p2.toString();
    }
    compareQueryParams(reqUrl, harUrlStr) {
    ...
    compareBody(reqBody, harPostData) {
        if (!harPostData || typeof harPostData.text !== 'string')
            return !reqBody;
            ...
            for (const sourcePath of this.sources) {
                try {
                    ...
                    if (stats.isDirectory()) {
                        const filesInDir = yield fs.readdir(sourcePath);
    ...
    _processSingleHarFile(harFilePath) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            try {
                const fileContent = yield fs.readFile(harFilePath, 'utf-8');
                const harJson = JSON.parse(fileContent);
                ...
                for (const entry of harJson.log.entries) {
                    if (!((_a = entry === null || entry === void 0 ? void 0 : entry.request) === null || _a === void 0 ? void 0 : _a.url) || !entry.response)
                        continue;
                    const method = entry.request.method.toUpperCase();
                    const urlObject = new URL(entry.request.url, 'http://dummy.base');
                    const urlPath = urlObject.pathname;
                    const pathMap = this.harDataMap.get(method) || new Map();
                    if (!this.harDataMap.has(method))
                        this.harDataMap.set(method, pathMap);
                    const state = pathMap.get(urlPath) || { entries: [], currentIndex: 0 };
                    if (!pathMap.has(urlPath))
                        pathMap.set(urlPath, state);
    ...
    loadHars() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.loadedHarFiles = yield this._resolveHarFilePaths();
            ...
            for (const filePath of this.loadedHarFiles) {
                yield this._processSingleHarFile(filePath);
            }
    ...
    printSummary() {
        ...
        console.log(`[INFO] Last reload at: ${(_a = this.lastReloadTimestamp) === null || _a === void 0 ? void 0 : _a.toLocaleString()}`);
        console.log(chalk.bold("------------------------\n"));
    }
    _reloadAllHars() {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    _serveManagementUi(res) {
        ...
        const renderEndpointList = (endpointList) => {
            if (endpointList.length === 0)
                return `<li class="empty-state">No endpoints loaded.</li>`;
...
<script>
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            form.parse(req, (err, fields, files) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (!uploadedFile || uploadedFile.size === 0) {
                    ...
                    if (uploadedFile === null || uploadedFile === void 0 ? void 0 : uploadedFile.filepath) {
                        yield fs.unlink(uploadedFile.filepath).catch(() => { });
                    }
                ...
                try {
                    ...
                    yield fs.rename(uploadedFile.filepath, newPath);
    ...
    sendResponse(res, entry) {
        ...
        if ((_a = response.content) === null || _a === void 0 ? void 0 : _a.text) {
    ...
    start() {
        return __awaiter(this, void 0, void 0, function* () {
            yield fs.mkdir(this.storageDir, { recursive: true });
            yield this.loadHars();
